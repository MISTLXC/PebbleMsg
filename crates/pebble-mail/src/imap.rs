...
impl ImapProvider {
    ...
    ///
    /// The logout command is wrapped in a short timeout to prevent the session
    /// mutex from being held indefinitely when the underlying TCP connection
    /// is already dead (common in LAN / unstable-network environments).
    pub async fn disconnect(&self) -> Result<()> {
        const LOGOUT_TIMEOUT_SECS: u64 = 5;
        let mut guard = self.session.lock().await;
        if let Some(sess) = guard.as_mut() {
            let logout_result = tokio::time::timeout(
                Duration::from_secs(LOGOUT_TIMEOUT_SECS),
                sess.logout(),
            )
            .await;
            match logout_result {
                Ok(Ok(())) => {
                    debug!("IMAP logout successful");
                }
                Ok(Err(e)) => {
                    debug!("IMAP logout returned error (session will be dropped): {e}");
                }
                Err(_) => {
                    debug!(
                        "IMAP logout timed out after {LOGOUT_TIMEOUT_SECS}s (session will be dropped)"
                    );
                }
            }
            *guard = None;
