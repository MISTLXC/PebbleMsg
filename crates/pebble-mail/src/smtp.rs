...
/// Maximum number of retries for transient SMTP failures.
const SMTP_MAX_RETRIES: u32 = 3;
/// Base delay between SMTP retries (seconds).
...
impl SmtpSender {
    ...
    pub async fn send(
        ...
        if let Some(ref proxy) = self.proxy {
            send_with_retry(|| self.send_via_proxy(&email, proxy)).await
        } else {
            send_with_retry(|| self.send_direct(&email)).await
        }
    }
...
/// Retry an SMTP send operation with exponential backoff.
async fn send_with_retry<F, Fut>(mut send_fn: F) -> Result<()>
where
    F: FnMut() -> Fut,
    Fut: std::future::Future<Output = Result<()>>,
{
    let mut last_error = None;
    for attempt in 0..=SMTP_MAX_RETRIES {
        match send_fn().await {
            Ok(()) => return Ok(()),
            Err(e) => {
                last_error = Some(e);
                if attempt < SMTP_MAX_RETRIES {
                    let delay = SMTP_RETRY_BASE_DELAY_SECS * 2u64.pow(attempt);
                    tokio::time::sleep(tokio::time::Duration::from_secs(delay)).await;
                }
            }
        }
    }
    Err(last_error.unwrap_or_else(|| PebbleError::Internal("SMTP send failed after retries".into())))
}
