...
fn is_retryable_imap_connection_error(error: &PebbleError) -> bool {
    ...
    lower.contains("os error 10053")
        ...
        || lower.contains("os error 10050")
        || lower.contains("os error 10060")
        || lower.contains("os error 10051")
        || lower.contains("os error 10065")
        ...
        || lower.contains("error in processing")
        || lower == "not connected"
}
...
fn is_missing_imap_mailbox_error(error: &PebbleError) -> bool {
    ...
    lower.contains("folder not exist")
        ...
        || lower.contains("folder could not be found")
}
...
pub(crate) struct SyncWorkerBase {
    ...
    /// Debounce repeated poll errors: folder key → last emit timestamp (secs).
    pub(crate) last_poll_error: std::sync::Mutex<HashMap<String, i64>>,
    /// Remote IDs of folders that were deleted from the server during this
    /// session. Prevents `sync_folders_from_server` from re-inserting them
    /// when the IMAP LIST cache is stale.
    pub(crate) deleted_remote_ids: std::sync::Mutex<HashSet<String>>,
}

impl SyncWorkerBase {
    /// Check whether we should emit a poll error for a folder, or skip it
    /// because the same error was already shown recently.
    fn should_emit_poll_error(&self, folder_name: &str) -> bool {
        const DEBOUNCE_SECS: i64 = 300; // 5 minutes
        let now = now_timestamp() as i64 / 1000; // millis → secs
        let mut map = self
            .last_poll_error
            .lock()
...
impl SyncWorker {
    ...
    pub fn new(
        ...
        Self {
            base: SyncWorkerBase {
                ...
                progress_tx: None,
                last_poll_error: std::sync::Mutex::new(HashMap::new()),
                deleted_remote_ids: std::sync::Mutex::new(HashSet::new()),
            },
            provider,
            idle_provider,
            stop_rx,
    ...
    async fn initial_sync_folder_with_reconnect(&self, folder: &pebble_core::Folder) -> Result<()> {
        match self.initial_sync_folder_once(folder).await {
            ...
            Err(e) => {
                // Non-retryable Network errors should still trigger a Session
                // cleanup — a dead TCP stream will poison all subsequent
                // initial-sync folder polls. Do not propagate the disconnect
                // result; if reconnection fails the remaining folders will
                // discover it via their own retryable-error branches.
    ...
    /// Number of successful poll cycles between IMAP folder-list refreshes.
    const FOLDER_SYNC_INTERVAL: u32 = 5;

    /// Periodically synchronise the folder list from the IMAP server into the
    /// local database. New folders discovered on the server are upserted;
    /// existing folders are left untouched.
    ///
    /// This is called at regular intervals during the main polling loop so that
    /// folder additions on the server are visible without an application restart.
    async fn sync_folders_from_server(&self) -> Result<()> {
        let remote_folders = self
            .provider
            .inner()
            .list_folders(&self.base.account_id)
        ...
        let deleted = self
            ...
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        for folder in &remote_folders {
            if deleted.contains(&folder.remote_id) {
                debug!(
                    "Skipping re-insert of folder '{}' (remote_id={}) — deleted during this session",
                    folder.name, folder.remote_id
                );
                continue;
            }
        ...
        debug!(
            "Synced {} folders from server for account {}",
            remote_folders.len(),
            self.base.account_id
        );
    ...
    /// Check whether enough poll cycles have passed to trigger a folder-list
    /// refresh from the server, and execute the refresh if needed.
    async fn maybe_sync_folders(&self, counter: &mut u32) {
        *counter += 1;
        if *counter >= Self::FOLDER_SYNC_INTERVAL {
            *counter = 0;
            if let Err(e) = self.sync_folders_from_server().await {
                warn!(
                    "Failed to sync folders from server for account {}: {}",
                    self.base.account_id, e
                );
            }
        }
    ...
    async fn poll_imap_folder_with_reconnect(
        ...
        match self.poll_imap_folder_once(folder).await {
            ...
            Err(e) if is_missing_imap_mailbox_error(&e) => {
                warn!(
                    "IMAP folder '{}' ({}) no longer exists on server, removing locally for account {}",
                    folder.name, folder.remote_id, self.base.account_id
                );
                // Save message IDs before the CASCADE delete removes
                // their folder associations. Orphaned messages (no other
                // folders) are soft-deleted — the server is the source
                // of truth and these messages no longer exist there.
                let orphaned_message_ids = self
                    .base
                    .store
                    .list_message_ids_for_folder(&self.base.account_id, &folder.id)
                    .unwrap_or_default();
                let _ = self
                    .base
                    .store
                    .delete_folder_by_remote_id(&self.base.account_id, &folder.remote_id);
                self.base
                    .deleted_remote_ids
                    .lock()
                    .unwrap_or_else(|poisoned| poisoned.into_inner())
                    .insert(folder.remote_id.clone());
                let mut deleted_count = 0usize;
                for msg_id in &orphaned_message_ids {
                    let remaining = self
                        .base
                        .store
                        .get_message_folder_ids(msg_id)
                        .unwrap_or_default();
                    if remaining.is_empty() {
                        let _ = self.base.store.soft_delete_message(msg_id);
                        deleted_count += 1;
                    }
                }
                if deleted_count > 0 {
                    info!(
                        "Soft-deleted {} orphaned messages from deleted folder '{}' for account {}",
                        deleted_count, folder.name, self.base.account_id
                    );
                }
                self.base.emit_error(
                    "folder_removed",
                    &format!(
                        "Folder '{}' was removed from the server and has been cleared locally.",
                        folder.name
                    ),
                );
                None
            ...
            Err(e) => {
                ...
                // If this is a Network error that wasn't classified as
                // retryable or missing-mailbox above, the session may be
                // dead. Disconnect so the next poll cycle reconnects
                // instead of reusing a broken connection.
                if matches!(e, PebbleError::Network(..)) {
                    let _ = self.provider.disconnect().await;
                }
                // Suppress repeated error notifications for the same folder
                if self.base.should_emit_poll_error(&folder.name) {
                    self.base.emit_error(
                        "poll",
    ...
    async fn reconcile_folder(&self, folder: &pebble_core::Folder) -> Result<()> {
        match self.reconcile_folder_once(folder).await {
            ...
            Err(e) if is_missing_imap_mailbox_error(&e) => {
                warn!(
                    "IMAP folder '{}' ({}) no longer exists on server during reconcile, removing locally for account {}",
                    folder.name, folder.remote_id, self.base.account_id
                );
                let orphaned_message_ids = self
                    .base
                ...
                if deleted_count > 0 {
                    info!(
                        "Reconcile: soft-deleted {} orphaned messages from deleted folder '{}' for account {}",
                        deleted_count, folder.name, self.base.account_id
                    );
                }
                self.base.emit_error(
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if backoff.is_circuit_open() {
                    ...
                    warn!(
                        "IMAP IDLE watcher circuit open for account {} ({} failures), waiting {:?} before half-open attempt",
                        account_id,
                    ...
                    // Half-open: attempt reconnection
                    warn!(
                        "IMAP IDLE watcher attempting half-open reconnect for account {}",
                        account_id
                    );
                    match idle_provider.connect().await {
                        ...
                        Err(e) => {
                            warn!(
                                "IMAP IDLE watcher half-open reconnect failed for account {}: {}",
                                account_id, e
                            );
    ...
    pub async fn run(
        ...
        // Connect with retry, then do initial sync
        let initial_sync_succeeded = {
            ...
            loop {
                ...
                if let Err(e) = self.provider.connect().await {
                    error!(
                        "Failed to connect for account {} (attempt {}): {}",
                        self.base.account_id,
                    ...
                    let delay = backoff.record_failure();
                    // Wait with backoff before retrying, or exit if manual-only
                    if config.manual_only() {
                        return;
                    }
                    match tokio::time::timeout(delay, stop_rx.changed()).await {
                        Ok(Ok(())) if *stop_rx.borrow() => return,
                        _ => continue,
                    }
                }
                ...
                match self.initial_sync().await {
                    ...
                    Err(e) => {
                        error!(
                            "Initial sync failed for account {} (attempt {}): {}",
                            self.base.account_id,
                            backoff.failure_count() + 1,
                            e
                        );
        ...
        let mut folder_sync_counter: u32 = 0;
                    ...
                    match trigger {
                        Some(trigger) => {
                            ...
                            let poll_result = if trigger == SyncTrigger::Manual {
                                // Manual trigger doubles as a repair button.
                                // If there are signs of a broken connection or
                                // corrupted state, perform a soft reset first.
                                let needs_repair = backoff.failure_count() > 0
                                    || !polling_baseline_trusted;
                                if needs_repair {
                                    let _ = self.provider.disconnect().await;
                                    if let Err(reconn_err) = self.provider.connect().await {
                                        warn!(
                                            "Manual trigger reconnect failed for account {}: {}",
                                            self.base.account_id, reconn_err
                                        );
                                    }
                                    backoff.record_success();
...
mod tests {
    ...
    fn imap_windows_connection_reset_is_retryable_for_polling() {
        let error = pebble_core::PebbleError::Network(
            "SELECT failed: io: 远程主机强迫关闭了一个现有的连接。 (os error 10054)"
                .to_string(),
        );
    ...
    #[test]
    fn imap_windows_connection_reset_en_is_retryable_for_polling() {
        let error = pebble_core::PebbleError::Network(
            "SELECT failed: io: An existing connection was forcibly closed by the remote host. (os error 10054)"
                .to_string(),
        );
    ...
    #[test]
    fn imap_network_unreachable_is_retryable_for_polling() {
        let error = pebble_core::PebbleError::Network(
            "TCP connect to 10.0.0.1:143: A socket operation was attempted to an unreachable host. (os error 10065)"
                .to_string(),
        );
    ...
    #[test]
    fn imap_network_down_is_retryable_for_polling() {
        let error = pebble_core::PebbleError::Network(
            "TCP connect to 10.0.0.1:143: A socket operation was attempted to an unreachable network. (os error 10051)"
                .to_string(),
        );
    ...
    #[test]
    fn imap_os_error_10060_timeout_is_retryable_for_polling() {
        let error = pebble_core::PebbleError::Network(
            "SELECT failed: io: A connection attempt failed because the connected party did not properly respond after a period of time. (os error 10060)"
                .to_string(),
        );
