...
use pebble_core::{now_timestamp, FolderRole, PebbleError};
...
fn build_popup_message(stored: &pebble_mail::StoredMessage) -> PopupMessage {
    let sender = if stored.message.from_name.trim().is_empty() {
        stored.message.from_address.clone()
    } else {
        stored.message.from_name.clone()
    };
    let subject = if stored.message.subject.trim().is_empty() {
        "(No subject)".to_string()
    } else {
        stored.message.subject.clone()
    };
    let snippet = stored.message.snippet.clone();
    let body_text = stored.message.body_text.clone();
    let sender_address = stored.message.from_address.clone();

    PopupMessage {
        message_id: stored.message.id.clone(),
        account_id: stored.message.account_id.clone(),
        subject,
        sender_name: sender,
        sender_address,
        snippet,
        body_text,
        received_at: stored.message.date,
        is_starred: stored.message.is_starred,
        is_read: stored.message.is_read,
...
fn show_new_mail_notification(
    ...
    let popup_msg = build_popup_message(stored);

    if let Some(state) = app.try_state::<AppState>() {
        state.popup_manager.show_popup(app, popup_msg)
    } else {
    ...
    match should_send_new_mail_notification(store, stored) {
        Ok(true) => {
            if let Err(e) = show_new_mail_notification(app, stored) {
                warn!("Failed to show new mail popup: {e}");
            }
