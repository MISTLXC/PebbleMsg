...
// ── Sync watchdog: auto-restart crashed sync workers with exponential backoff ──
...
impl SyncBackoffState {
    fn backoff_delay(&self) -> u64 {
        let failures = self.consecutive_failures.min(MAX_CONSECUTIVE_RESTARTS);
        let delay = BASE_BACKOFF_SECS * 2u64.pow(failures);
...
impl Default for SyncBackoffState {
    fn default() -> Self {
        Self {
...
/// Background task that periodically checks for dead sync workers and
/// restarts them with exponential backoff. This provides self-healing
/// for transient network failures and server instability.
pub async fn run_sync_watchdog(app: tauri::AppHandle) {
    use tauri::Manager;
    let backoff_states: Arc<TokioMutex<std::collections::HashMap<String, SyncBackoffState>>> =
    ...
    loop {
        ...
        let poll_interval = {
            // Read the current realtime preference from the store, default to 3s
            3u64
        };
        ...
        for account in accounts {
            ...
            if !is_dead {
                // Sync worker is alive — clear backoff state on success
                let mut states = backoff_states.lock().await;
                if let Some(bs) = states.get_mut(&account_id) {
                    bs.record_success();
                }
                continue;
            }

            // Sync worker is dead — check if we can retry
            let can_retry = {
                let mut states = backoff_states.lock().await;
                let bs = states.entry(account_id.clone()).or_default();
                if !bs.can_retry() {
                    let next = bs.next_retry_at.map(|t| {
                    ...
                    emit_realtime_status(
                        ...
                        realtime_status_payload(
                            ...
                            bs.last_attempt_at.map(|t| {
                                t.duration_since(UNIX_EPOCH)
                                    .unwrap_or_default()
                                    .as_secs() as i64
                            }),
                            next,
                            Some(format!("Retrying in {}s", bs.backoff_delay())),
                        ),
                    );
                    false
                } else {
            ...
            info!(
                "Sync watchdog: restarting sync for account {}",
                account_id
            );
            if let Err(e) = start_sync_inner(&app, &state, account_id.clone(), Some(poll_interval)).await {
                warn!(
                    "Sync watchdog: failed to restart sync for account {}: {}",
                    account_id, e
                );
            }
        }
...
fn realtime_error_mode(sync_error: &pebble_mail::SyncError) -> RealtimeMode {
        ...
        || text.contains("401")
    {
        ...
        // Classify transient protocol/connection errors as Backoff so they
        // are retried automatically rather than treated as permanent errors.
        // Covers IMAP "bad response", "SELECT failed", connection reset, etc.
        if text.contains("bad response")
            || text.contains("select failed")
