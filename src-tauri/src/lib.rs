...
pub fn run() {
    ...
    builder
        ...
        .setup(|app| {
            ...
            // Load persisted popup config and apply auto-launch
            {
                let state: tauri::State<AppState> = app.state();
                if let Ok(Some(bytes)) = state.store.get_secure_user_data("popup_config") {
                    if let Ok(config) = serde_json::from_slice::<popup::PopupConfig>(&bytes) {
                        let auto_launch = config.auto_launch;
                        let minimize_on_startup = config.minimize_on_startup;
                        state.popup_manager.update_config(config);
                        // Apply auto-launch registry at startup (ensure it matches config)
                        let _ = crate::commands::popup::apply_auto_launch_registry(auto_launch);
            ...
            log_startup_phase(startup_start, &mut startup_phase, "popup config loaded");
            ...
            // Start the sync watchdog for auto-recovery on transient failures
            let app_for_watchdog = app_handle.clone();
            tauri::async_runtime::spawn(async move {
                commands::sync_cmd::run_sync_watchdog(app_for_watchdog).await;
            });
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::popup::update_popup_config,
            commands::popup::get_popup_config,
            commands::popup::get_popup_data,
            commands::popup::show_test_popup,
            commands::popup::close_popup,
            commands::popup::popup_open_message,
            commands::popup::set_ui_theme,
